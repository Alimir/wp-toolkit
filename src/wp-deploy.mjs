...
import { commandExists, run, runQuiet } from './utils.mjs';

export async function releaseProject() {
	const context = getContext();
	const { paths, pkg, wpDeploy, slug } = context;

	if (!wpDeploy.enabled) {
		throw new Error('WordPress.org release is disabled in wp-toolkit.config.mjs (release.enabled: false).');
	}

	const env = requireEnv(paths.root, ['WP_SVN_USER', 'WP_SVN_URL']);
	assertSafeSvnConfig(wpDeploy);

	if (!existsSync(paths.buildPath)) {
		throw new Error(`Build directory not found at ${paths.buildPath}. Run "wp-toolkit build" first.`);
	}

	assertReleaseBuild(paths.buildPath, context);
	...
	const workDir = join(tmpdir(), `${slug}-svn`);
	...
	function assertReadmeStableTag() {
		if (!context.validation.checkStableTag) {
			return;
		}
		...
		if (existsSync(releaseTagDir)) {
			throw new Error(
				[
					...
					`Bump the version in package.json, ${context.mainFile}, and readme.txt before releasing again.`,
					'If you only need to update trunk without a new tag, that is not supported by wp-toolkit release.',
				].join('\n')
			);
		}
	}
	...
	async function confirmRelease() {
		...
		const answer = await rl.question(
			`\nPublish ${slug} ${pkg.version} to WordPress.org?\n` +
	...
	async function publishRelease() {
		const commitMessage = process.env.WP_RELEASE_MESSAGE || `Updating to ${pkg.version}`;
		const stageTargets = wpDeploy.assetsDir ? ['trunk', 'assets'] : ['trunk'];
